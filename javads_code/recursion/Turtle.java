...
public class Turtle {
	...
	private Point2D.Double location; // screen coordinates
    private Point2D.Double turtleCoords; // turtle coordinates
	private boolean isdown;
	private Path2D.Double fillPath = null;
	private Color color;
	private double shellSize;
	private int pause = 200;
    private boolean visible = true;
    private boolean filling = false; // is path to be filled in?
	...
	public Turtle(World w, double x, double y) {
        this.turtleCoords = new Point2D.Double(x, y);
		this.location = new Point2D.Double(
            w.getScaleX() * x + w.getOffsetX(),
            w.getScaleY() * y + w.getOffsetY());
        /* System.err.printf("Turtle initial location %s%n", this.location.toString()); */
		this.theta = 0;
		this.world = w;
    ...
    /**
     * Check the turtle state
     * @return true if turtle is visible, false otherwise
     */
    ...
    /**
     * Convenience routine to hide the turtle
     */
    public void hide() {
    ...
    /**
     * Convenience routine to show the turtle
     */
    public void show() {
	...
	public double getHeading() {
		return Math.toDegrees(theta); // was theta * 180 / Math.PI;
	}
	...
	public void setHeading(double angle) {
		this.theta = Math.toRadians(angle); // was angle * Math.PI / 180;
		world.turtleMoved();
		this.pause();
	}
	
    /**
     * Find angle between current location and given coordinates
     * @param x destination x coordinate
     * @param y destination y coordinate
     */
    public double towards(double x, double y) {
        double radians = Math.atan2(x - this.turtleCoords.x,
            y - this.turtleCoords.y);
	...
	protected void cornerGoTo(double x, double y) {
        ...
        this.turtleCoords.x = x;
        this.turtleCoords.y = y;
		this.location.x = this.world.getScaleX() * x + this.world.getOffsetX();
		this.location.y = this.world.getScaleY() * y + this.world.getOffsetY();
        /* System.err.printf("turtle (%.1f, %.1f) --> screen(%.1f, %.1f).%n",
            x, y, this.location.x, this.location.y); */
		if (this.isdown) {
			world.drawLine(this.location, ox, oy, this.penWidth, this.color);
            if (filling) {
                this.fillPath.lineTo(this.location.x, this.location.y);
            }
			world.turtleMoved();
			this.pause();
		} else {
            if (filling) {
                this.fillPath.moveTo(this.location.x, this.location.y);
            }
			world.turtleMoved();
			this.pause();
		}
    ...
    /**
     * Begin a path that will be filled in the current color
     */
    public void beginFill() {
        this.fillPath = new Path2D.Double();
        this.fillPath.moveTo(this.location.x, this.location.y);
        this.filling = true;
    }
    
    /**
     * End a path that will be filled
     */
    public void endFill() {
        this.filling = false;
        this.world.fill(fillPath, fillColor);
    }
    
    /**
     * Set color for filling paths
     */
    ...
    /**
     * Retrieve color for filling paths
